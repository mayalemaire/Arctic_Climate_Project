...
data_folder <- "Arctic_Climate_Project/output/ROS_3mm_1t2m_sd_m3d"
...
alaska_data$rf <- alaska_data$rf * 1000
northern_fennoscandia_data$rf <- northern_fennoscandia_data$rf * 1000
svalbard_data$rf <- svalbard_data$rf * 1000
yamal_data$rf <- yamal_data$rf * 1000
...
ROS_intensity_combined <- ggplot(combined_data_ROS, aes(x = start_date, y = total_rf, size = event_length, color = intensity)) +
  ...
  scale_size_continuous(
    range = c(1, 5), 
    breaks = c(1, 2, 3, 5, 10, 15),  # Explicit breaks for whole days
    labels = function(x) paste0(x, " day", ifelse(x > 1, "s", ""))  # Ensure correct singular/plural form
  ) +
  ...
  geom_vline(
    xintercept = seq(as.Date("1950-01-01"), as.Date("2021-01-01"), by = "5 years"),
    linetype = "dashed",
    color = "grey"
  )  # Add lines every 5 years
